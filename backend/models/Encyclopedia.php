...
class Encyclopedia extends \yii\db\ActiveRecord
{
    ...
    {
        if ($this->validate()) {
            $this->imageFile->saveAs(Yii::getAlias('@frontend/web/images/encyclopedia/'). $url . '.' . $this->imageFile->extension);
    ...
    public static function delImg($img){
        return @unlink(Yii::getAlias('@frontend/web/images/encyclopedia/').$img);
    }
}
